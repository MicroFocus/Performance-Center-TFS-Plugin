...
namespace PC.Plugins.Configurator
{
    public class Configurator
    {
        ...
        public static bool IsLogFileEnded(string fullFilename)
        {
            try
            {
                if (File.Exists(fullFilename))
                {
                    ...
                    return fileContent.EndsWith(END_OF_LOG_FILE) || fileContent.Contains(END_OF_LOG_FILE);
                }
                else
                    return false;
            }
        ...
        public static string GetNewContent(string fullFilename)
        {
            ...
            if (File.Exists(fullFilename))
            {
                if (File.Exists(FullFileNamePreviouslyRead))
                    ReadAllLinesFromPosition(FullFileNamePreviouslyRead, ref lastPositionPreviouslyRead);

                File.Copy(fullFilename, FullFilenameDuplicated, true);
                newContentArray = ReadAllLinesFromPosition(FullFilenameDuplicated, ref lastPositionPreviouslyRead);
                newContent = newContentArray.Length > 0 ? String.Join("\r\n", newContentArray) : "";
                File.Copy(FullFilenameDuplicated, FullFileNamePreviouslyRead, true);
            }
            return newContent;
        }
        ...
        public static void DeleteUnusedFilesFromArtifact(string logFullFilename)
        {
            ...
            string logFullFileNameDuplicated = Path.Combine(logFullFilename + DUPLICATED);
            try
            {
                if (File.Exists(logFullFileNamePreviouslyRead))
                    File.Delete(logFullFileNamePreviouslyRead);
                if (File.Exists(logFullFileNameDuplicated))
                    File.Delete(logFullFileNameDuplicated);
            }
            catch { }
        }
